-- 0193 — a run item pointed at the tombstone rather than the row.
--
-- ## The consequence of `0192`, found by finalizing
--
-- `0192` made uniqueness a property of live observations so a redacted row
-- stops blocking its replacement. It worked — Demo's reconnect produced 1,500
-- observations where the previous attempt produced none. Then finalizing that
-- run raised:
--
--     23505 duplicate key value violates unique constraint
--           "observations_live_fingerprint_key"
--
-- `finalize_ingestion_run_v031` sets an item's observation back to `active`
-- when the item is `present`. The item pointed at the **tombstone**, and
-- reviving it collided with the live row holding the same fingerprint.
--
-- **Measured on that run: 1,352 of 1,435 items pointed at a deleted
-- observation, 83 at the live one.** The item insert resolves the observation
-- by `(user_id, source_code, record_fingerprint)` with no lifecycle filter, so
-- once a tombstone and its replacement both exist it takes whichever the
-- planner reaches first.
--
-- **The asymmetry was already visible in the same statement.** The raw-record
-- join beside it insists on `r.lifecycle_state = 'active'`; the observation
-- join insisted on nothing. `0192` is what made that matter.
--
-- ## What this does not repair, and why
--
-- `ingestion_run_items` refuses every operation that is not an `INSERT`, so the
-- 1,352 wrong pointers cannot be corrected — that immutability is the point of
-- the table and is worth more than this repair. So Demo's 15:19 run stays
-- `running` forever, inert.
--
-- **That costs nothing it appears to cost.** The resolver reads observations,
-- not runs, and those 1,500 are `active`: they are scored either way. What the
-- unfinalized run withholds is the automatic recompute enqueue, which is one
-- job, and the next distillation opens a clean run that finalizes normally.
...
begin
  ...
  with written as (
    ...
    left join semantic_private.observations o
     ...
     -- **Live rows only, as the raw join above already insists.** Since `0192`
     -- a redacted observation and its replacement share a fingerprint, and
     -- without this the item points at whichever the planner reached first —
     -- measured: 1,352 of 1,435 items pointed at tombstones. Finalization then
     -- tries to revive a tombstone whose key the live row holds and fails on
     -- the unique index, so the run can never close.
...
declare
  pointing integer;
begin
  -- The state this migration describes, still measurable: the run that found
  -- the defect. If it is gone the migration is being replayed against a
  -- different database, and the assertion below would be describing nothing.
  select count(*) into pointing
    from semantic_private.ingestion_run_items i
    join semantic_private.observations o on o.id = i.observation_id
   where o.lifecycle_state = 'deleted';
  raise notice '0193: % existing run item(s) point at a redacted observation, unrepairable by design',
    pointing;

  -- **A source check, and named as the weak thing it is.** The behaviour cannot
  -- be exercised here: it needs an ingestion call with a live tombstone pair,
  -- which is the client's half. What is checkable is that the join carries the
  -- filter at all.
  if not exists (
    select 1 from pg_proc p
     ...
     where n.nspname = 'semantic_private'
       ...
       and p.prosrc like '%and o.lifecycle_state <> ''deleted''%'
  ) then
    raise exception '0193: the observation join is not scoped to live rows';
  end if;
end;
$$;
