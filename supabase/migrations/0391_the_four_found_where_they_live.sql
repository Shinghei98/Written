-- 0391 — the same four, found where the fracture actually put them.
--
-- 0390 aimed true and hit air: each target lived one identity away.
-- 1. **Sakura's concept label itself carries the composed decorated
--    string** — the kept mint stored a display form as the name; the
--    decoration strips at the revision and label, generically.
-- 2. **Next Level has only the member stated** — no aespa relation
--    exists; the member's own member_of_group resolves the group,
--    two-hop, deterministic.
-- 3. **The agencies' relations point at their franchise-family sibling
--    rows** — classification aggregates across every dictionary row
--    sharing the organization's name.
-- 4. **THE BOOK 3's album row promoted to a different concept than the
--    page's** — evidence decides, family does not: any promoted concept
--    whose label an observation's album field names follows that row's
--    performer.
...
begin
  ...
  values (extensions.gen_random_uuid(), next_version, old_version_id, 'draft',
          'The four page-audit rules, applied where the identities really live.');
  ...
  -- 1. Concept names shed channel decorations and composed parentheses.
  update ontology.concept_revisions r
     set preferred_label = btrim(regexp_replace(regexp_replace(
           r.preferred_label, '\s*[（(][^）)]*[）)]\s*$', ''),
   ...
   where r.ontology_version_id = new_version_id and r.status = 'active'
     ...
     and (r.preferred_label ~ '^(ちゃんねる|チャンネル|[Cc]hannel)\s*\S'
          or r.preferred_label ~ '[（(](ちゃんねる|チャンネル|[Cc]hannel)')
     and btrim(regexp_replace(regexp_replace(
           r.preferred_label, '\s*[（(][^）)]*[）)]\s*$', ''),
           '^(ちゃんねる|チャンネル|[Cc]hannel)\s*', '')) <> '';
  get diagnostics n = row_count;
  raise notice '0391: % creator name(s) shed decorations', n;

  if music_id is not null then
    -- 3. Shelving across name-siblings.
    create temporary table _orgs on commit drop as
    select c.id, t.normalized_label,
    ...
    select new_version_id, o.id, 'broader',
           ...
           0.8, 'learned', '{"source":"0391_label_shelves"}'::jsonb, 'active'
      from _orgs o where o.music_feeders > 0 or o.screen_feeders > 0
    on conflict do nothing;
    get diagnostics n = row_count;
    raise notice '0391: % organization(s) shelved across siblings', n;
  end if;

  -- 4. Evidence decides, family does not.
  insert into ontology.concept_edges (
    ontology_version_id, subject_concept_id, predicate_key, object_concept_id,
    confidence, provenance_type, provenance, status)
  select distinct on (ac.id)
         new_version_id, ac.id, 'broader', h.id, 0.75, 'learned',
         '{"source":"0391_album_follows_evidence"}'::jsonb, 'active'
    from ontology.concept_revisions ar
    join ontology.concepts ac on ac.id = ar.concept_id
    ...
    join ontology.concepts pc on pc.id = pl.concept_id and pc.id <> ac.id
    cross join lateral (
   ...
   where ar.ontology_version_id = new_version_id and ar.status = 'active'
     and ar.concept_kind = 'work'
     and not exists (select 1 from ontology.concept_edges e
       where e.ontology_version_id = new_version_id and e.status='active'
  ...
  on conflict do nothing;
  get diagnostics n = row_count;
  raise notice '0391: % work(s) follow their evidence performer', n;

  perform ontology.publish_version(new_version_id);
  perform semantic_private.enqueue_recompute_on_analysis_change(
    'ontology ' || next_version || ': the four rules, landed where the identities live');
  raise notice '0391: % published', next_version;
end;
$$;

-- 2. The two-hop group resolution.
...
begin
  if position('order by coalesce(score.surfacing_score, 1.0) desc' in
    ...
    raise exception '0391: list_assertions lost its ordering';
  end if;
end;
$$;
