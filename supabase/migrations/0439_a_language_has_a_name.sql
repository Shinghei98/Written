-- 0439 — a language has a name, and the general term yields to it.
--
-- **The owner's refinement (2026-08-27): "for language learning, be
-- specific if possible."** The signal was always specific — the
-- channel's own tags say "French|French Lessons|A1 French" — but the
-- curated vocabulary had no per-language subjects, so the whole-tag
-- match could only reach the general `subject:language_learning`.
--
-- Three parts, each a rule:
--   * **An authored language-name registry** (the curated-source
--     pattern: a small closed vocabulary, not a guess) names the
--     languages a tag may claim.
--   * **Per-language subjects mint under language_learning** — only
--     for languages a language-learning channel's tags actually name,
--     labeled "French (language)" so the label collides with nothing;
--     the channel gets the specific edge beside its general one, and
--     propagation does the rest on the enqueued run.
--   * **"Be specific if possible" becomes a display rule**: a field
--     parent is withheld while a shown field child points at it — so
--     the general term appears exactly when nothing more specific
--     does, for languages and for every other field alike.

begin;

create table semantic_private.language_names (
  name text primary key,
  label text not null
);
alter table semantic_private.language_names enable row level security;

insert into semantic_private.language_names (name, label) values
  ('french', 'French'), ('korean', 'Korean'), ('japanese', 'Japanese'),
  ('mandarin', 'Mandarin'), ('cantonese', 'Cantonese'),
  ('chinese', 'Chinese'), ('spanish', 'Spanish'), ('german', 'German'),
  ('italian', 'Italian'), ('portuguese', 'Portuguese'),
  ('russian', 'Russian'), ('arabic', 'Arabic'), ('hindi', 'Hindi'),
  ('english', 'English'), ('vietnamese', 'Vietnamese'),
  ('thai', 'Thai'), ('indonesian', 'Indonesian'), ('turkish', 'Turkish'),
  ('polish', 'Polish'), ('dutch', 'Dutch'), ('swedish', 'Swedish'),
  ('greek', 'Greek'), ('hebrew', 'Hebrew'), ('latin', 'Latin');
...
begin
  ...
  values (extensions.gen_random_uuid(), next_version, old_version_id, 'draft',
          '0439: per-language subjects under language_learning');
  ...
  select id into ll_id from ontology.concepts
   where concept_key = 'subject:language_learning';

  -- The languages actually named by language-learning channels' tags.
  create temporary table _langs on commit drop as
  select distinct e.subject_concept_id as channel_id,
         ln.name, ln.label,
         'subject:' || replace(ln.name, ' ', '_') || '_language'
           as lang_key
    from ontology.concept_edges e
    join public.summary_distilled_records d
      on d.source = 'youtube' and d.data_type = 'subscription'
     and coalesce(d.extra ->> 'keywords', '') <> ''
    join ontology.concept_labels cl
      on cl.concept_id = e.subject_concept_id
     and cl.ontology_version_id = new_version_id and cl.status = 'active'
     and cl.normalized_label = lower(d.name)
    cross join lateral unnest(string_to_array(d.extra ->> 'keywords', '|'))
      as t(tag)
    join semantic_private.language_names ln
      on ln.name = lower(replace(btrim(t.tag), '_', ' '))
   where e.ontology_version_id = new_version_id
     and e.predicate_key = 'broader' and e.status = 'active'
     and e.object_concept_id = ll_id;

  insert into ontology.concepts (id, concept_key)
  select extensions.gen_random_uuid(), l.lang_key
    from (select distinct lang_key from _langs) l
  ...
  select distinct on (c.id)
         new_version_id, c.id, l.label || ' (language)', 'topic',
         'ordinary', 'inferable', 'active', '{}'::jsonb
    from (select distinct lang_key, label from _langs) l
    join ontology.concepts c on c.concept_key = l.lang_key
  ...
  select distinct on (c.id)
         new_version_id, c.id, l.label || ' (language)',
         lower(l.label || ' (language)'), 'en', 'preferred', 'curated',
         1.0, 'active'
    from (select distinct lang_key, label from _langs) l
    join ontology.concepts c on c.concept_key = l.lang_key
  on conflict do nothing;

  -- The language files under language learning, and the channel under
  -- the language.
  ...
  select distinct new_version_id, c.id, 'broader', ll_id, 0.9, 'curated',
         jsonb_build_object('source', '0439_language_registry'), 'active'
    from (select distinct lang_key from _langs) l
    join ontology.concepts c on c.concept_key = l.lang_key
   where not exists (
     select 1 from ontology.concept_edges e
      where e.ontology_version_id = new_version_id
        ...
        and e.object_concept_id = ll_id
        and e.provenance_type = 'curated');

  insert into ontology.concept_edges
    (ontology_version_id, subject_concept_id, predicate_key,
     object_concept_id, confidence, provenance_type, provenance, status)
  select distinct new_version_id, lg.channel_id, 'broader', c.id, 0.7,
         'provider',
         jsonb_build_object('source', 'uploader_tags',
                            'rule', '0439 language tag'), 'active'
    from _langs lg
    join ontology.concepts c on c.concept_key = lg.lang_key
   where not exists (
     select 1 from ontology.concept_edges e
      where e.ontology_version_id = new_version_id
        and e.subject_concept_id = lg.channel_id
        and e.predicate_key = 'broader'
        and e.object_concept_id = c.id
        and e.provenance_type = 'provider');
  get diagnostics n = row_count;
  raise notice '0439: % channel-to-language edge(s)', n;

  perform ontology.publish_version(new_version_id);
  raise notice '0439: % published', next_version;
end;
$$;
...
begin
  perform semantic_private.enqueue_recompute_on_analysis_change(
    '0439: languages get names; the general term yields to the specific');
end;
$$;
