-- 0477 — a person is one or more of nine kinds, and is shown as the heaviest.
--
-- **The owner's categories (2026-09-08), replacing 0342's twelve:**
-- performer (singer, dancer, musician), composer, author (book or blog),
-- host (TV or radio presenter, anchor), athlete, actor, director, streamer,
-- content_creator (a YouTube or TikTok channel personality who is not a
-- streamer). A person may occupy more than one; the page shows the one with
-- the most weight. `character` is removed: a fictional character is not a
-- person, and the work it belongs to is the term — the definition the model
-- reads says so from qwen_extractor_v25 on. `comedian` folds into performer,
-- `music_performer` is performer; `artist` and `historical_figure` are not
-- on the list and their persons are held with no category, as 0342 holds a
-- person the pass could not place.
--
-- Why the list changed. Loki and Lucifer reached David's page as creators:
-- the dictionary held both as `person` with `person_subtype = character`,
-- a kind the schema admitted and the definition never excluded, so the
-- model typed a series by its title character and the keep route minted
-- the family it was given. The same pass filed 195 rows as characters,
-- and the sample is mostly real musicians and creators (Aexgrant, AiRPLAY,
-- Call Me Karizma) — the pass reached for the word because it was there.
-- Those rows lose the subtype and are asked again under the nine, with a
-- ranked answer; they are not retyped by this file, which knows only that
-- the old answer was wrong, not what the right one is.
--
-- The shape. `person_category_support (term, category, support, source)`
-- holds every answer with its weight; `presumed_terms.person_subtype` is
-- the argmax, maintained by trigger, so every reader of the old column
-- keeps working and reads the heaviest. Existing answers seed the table at
-- support 1.0 under their new names. `api.list_assertions` carries the
-- category on `display_payload` as `person_category`. The 0369/0386 bridge
-- lists that keyed on the old names are one-off passes; the next bridge
-- reads: screen-shaped = actor, director, host, streamer, content_creator.
begin;

-- ---------------------------------------------------------------------------
-- 1. The dictionary's answers under their new names; the removed ones held.
-- ---------------------------------------------------------------------------
alter table semantic_private.presumed_terms
  drop constraint if exists presumed_terms_person_subtype_check;

update semantic_private.presumed_terms
   set person_subtype = 'performer'
 where person_subtype in ('music_performer', 'comedian');

update semantic_private.presumed_terms
   set person_subtype = null, person_subtype_source = null
 where person_subtype in ('character', 'artist', 'historical_figure');

alter table semantic_private.presumed_terms
  add constraint presumed_terms_person_subtype_check
...
-- ---------------------------------------------------------------------------
-- 2. Every answer with its weight; the column is the heaviest.
-- ---------------------------------------------------------------------------
create table if not exists semantic_private.person_category_support (
  term_id     uuid not null references semantic_private.presumed_terms (id),
  category    text not null check (category in (
                'performer', 'composer', 'author', 'host', 'athlete',
                'actor', 'director', 'streamer', 'content_creator')),
  support     numeric(5, 3) not null check (support >= 0 and support <= 1),
  source      text not null check (source in ('model_pass', 'authored', 'derived')),
  created_at  timestamptz not null default now(),
  primary key (term_id, category)
);
alter table semantic_private.person_category_support enable row level security;
grant select, insert, update on semantic_private.person_category_support to semantic_worker;

create or replace function semantic_private.refresh_person_subtype(p_term uuid)
returns void
language sql
set search_path = ''
as $function$
  update semantic_private.presumed_terms t
     set person_subtype = s.category,
         person_subtype_source = case s.source when 'authored' then 'authored'
                                               else 'model_pass' end
    from (select category, source
            from semantic_private.person_category_support
           where term_id = p_term
           order by support desc, created_at asc, category
           limit 1) s
   where t.id = p_term
     and (t.person_subtype is distinct from s.category
          or t.person_subtype_source is distinct from
             case s.source when 'authored' then 'authored' else 'model_pass' end);
$function$;

create or replace function semantic_private.person_category_support_refresh()
returns trigger
language plpgsql
set search_path = ''
as $function$
begin
  perform semantic_private.refresh_person_subtype(
    case when tg_op = 'DELETE' then old.term_id else new.term_id end);
  return null;
end;
$function$;

drop trigger if exists person_category_support_refresh on semantic_private.person_category_support;
create trigger person_category_support_refresh
  after insert or update or delete on semantic_private.person_category_support
  for each row execute function semantic_private.person_category_support_refresh();

-- Seed: what the dictionary already answers, at full support.
insert into semantic_private.person_category_support (term_id, category, support, source)
select t.id, t.person_subtype, 1.0,
       case t.person_subtype_source when 'authored' then 'authored' else 'model_pass' end
  from semantic_private.presumed_terms t
 where t.person_subtype is not null
on conflict (term_id, category) do nothing;

-- ---------------------------------------------------------------------------
-- 3. Proven both ways, on real rows, inside this transaction.
-- ---------------------------------------------------------------------------
do $$
declare
  probe uuid;
...
begin
  select id into probe from semantic_private.presumed_terms
   where family = 'person' and person_subtype is null limit 1;
  if probe is null then
    raise notice '0477: no unanswered person to probe with';
    return;
  end if;
  insert into semantic_private.person_category_support (term_id, category, support, source)
  values (probe, 'host', 0.4, 'derived'), (probe, 'performer', 0.9, 'derived');
  select person_subtype into read from semantic_private.presumed_terms where id = probe;
  if read <> 'performer' then
    raise exception '0477: the heaviest category did not become the subtype (%)', read;
  end if;
  update semantic_private.person_category_support set support = 0.95
   where term_id = probe and category = 'host';
  select person_subtype into read from semantic_private.presumed_terms where id = probe;
  if read <> 'host' then
    raise exception '0477: a heavier answer did not take over (%)', read;
  end if;
  begin
    insert into semantic_private.person_category_support (term_id, category, support, source)
    values (probe, 'character', 0.5, 'derived');
  exception when check_violation then
    refused := true;
  end;
  if not refused then
    raise exception '0477: character was admitted as a category';
  end if;
  delete from semantic_private.person_category_support where term_id = probe;
  select person_subtype into read from semantic_private.presumed_terms where id = probe;
  if read is not null then
    -- the trigger keeps the column only where an answer stands; with none
    -- left, the last winner is explicitly cleared
    update semantic_private.presumed_terms
       set person_subtype = null, person_subtype_source = null where id = probe;
  end if;
  raise notice '0477: the category support answered both ways';
end $$;

-- ---------------------------------------------------------------------------
-- 4. The reader carries the category.
-- ---------------------------------------------------------------------------
...
begin
  ...
  select
    ...
    -- 0477: a person's category rides on the payload — the heaviest of the
    -- owner's nine (performer, composer, author, host, athlete, actor,
    -- director, streamer, content_creator), read off the dictionary's
    -- support rows; null where none has been answered.
    score.display_payload
      || coalesce((
           select jsonb_build_object('person_category', t.person_subtype)
             from semantic_private.presumed_terms t
            where t.promoted_concept_id = assertion.concept_id
              and t.family = 'person' and t.person_subtype is not null
            order by t.mention_support desc nulls last, t.id
            limit 1), '{}'::jsonb),
    score.id,
