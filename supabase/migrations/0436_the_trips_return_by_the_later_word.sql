-- 0436 — the trips return to the page, by the owner's later word.
--
-- **Two owner rulings collided and the later one controls.** 0375
-- (owner, 2026-08-25) took place, travel and culture off Memories as
-- internal markings. Today (2026-08-27) the owner ruled the calendar
-- lane's terms belong on the page as ordinary terms — "a trip is a
-- term like any other" — and asked where they were. The supersession
-- is exactly as wide as the ruling: `travel:` keys are admitted by
-- name; the place and culture halves of 0375 stand, and 0367's
-- entity ruling on other activity concepts is not reopened.
--
-- The block machinery already files a trip under `subject:travel`
-- (0373), so the returning rows arrive under their own heading.
...
begin
  if position('order by coalesce(score.surfacing_score, 1.0) desc' in
    ...
    raise exception '0436: list_assertions lost its ordering';
  end if;
end;
$$;
