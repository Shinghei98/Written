-- 0441 — a rating is not a sentence, and a musical is not an album.
--
-- **Two leaks from the same clause, both the owner's catches.**
-- "Requiem" survived 0440 because the ratings import stamped it
-- `explicit_addition` and the clause guarded on `inferred` — but a
-- rated album is a data artifact, not something anybody typed (typed
-- terms are `user_term` rows and pass on their own branch). And
-- Wicked *vanished* under 0440, because `genre:musicals` lives in the
-- music hub — yet Wicked is the entity its soundtrack's songs point
-- at, with 36 relations flowing INTO it, where the recordings have
-- none. So the clause loses its origin qualifier and gains the
-- structural exemption: a music-hub work with inbound edges is a
-- container — a thing other things belong to — and stays; a leaf
-- nothing references is a recording, whose line belongs to its
-- performer.
...
begin
  if position('order by coalesce(score.surfacing_score, 1.0) desc' in
    ...
    raise exception '0441: list_assertions lost its ordering';
  end if;
end;
$$;
