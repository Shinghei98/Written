-- 0062 — the shadow comparison gets somewhere to live.
--
-- **Phase 1's last unfinished bullet: "Record old/new correlation IDs and
-- compare record/source/action coverage."** The correlation id has been on
-- every envelope since the envelope existed. The comparison was a `print`
-- inside `#if DEBUG`, which is not a record of anything — and it cost twice.
-- When a run came back with four data types instead of nine I could not tell
-- from the server whether the device had sent one batch or three; the device
-- knew and had nowhere to put it. When a `ReferenceError` broke every request,
-- the client saw 500s and queued them quietly while the vault sat empty.
--
-- **`ingestion_runs.metrics` is the home the schema already had** — `jsonb not
-- null default {}`, no constraints, empty on every run so far. No new table.
--
-- **It has to be written while the run is `running`.**
-- `guard_ingestion_run_update` refuses any change to a terminal run — *terminal
-- ingestion runs are immutable* — so metrics accumulate per batch and the
-- finalizer closes over whatever is there. That is also the right shape: a
-- number written after the fact is a number nobody can tie to the run that
-- produced it.
--
-- Two halves, kept apart because they answer different questions. `client` is
-- what the device believed it was sending — how many legacy rows the same
-- distillation produced, how many envelopes it could build, what it withheld
-- and what it could not describe. `server` is what actually landed. A
-- disagreement between them is the entire point of a shadow phase, and neither
-- half can report it alone.
--
...
revoke all on function semantic_private.ingest_source_records_v031(
  uuid, uuid, text, text, text, text, text, text, jsonb, jsonb, boolean, jsonb
...
-- `0052`'s claim, re-checked because the signature moved: one callable
-- function, and the old eleven-argument form gone rather than overloading it.
do $$
declare
  callable integer;
begin
  select pg_catalog.count(*) into callable
