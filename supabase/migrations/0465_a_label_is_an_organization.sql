-- 0465 — a label is an organization, not a work.
--
-- **The owner's direction (2026-08-28): music labels move to the
-- organization kind.** They had been minted as works (and JYP, earlier,
-- as a creator) because nothing in the corpus states what they are: the
-- dictionary's families call ATEEZ an organization and Pop/Stars one
-- too, the shelf holds groups and songs beside the labels with the same
-- learned edges, and 0461 already proved this class of identity is not
-- derivable in-corpus. So this is curation, JYP's own precedent — each
-- row below is a judgement with its name on it, not a rule pretending
-- to generality. What stays behind on the shelves, deliberately: the
-- K-pop groups (ATEEZ, TWS, TXT, LIGHTSUM, Rainbow — mis-kinded as
-- works, but they are creators, a different move for a different day),
-- the fandom (WIZONE), the songs and musicals (Pop/Stars, All Shook
-- Up), and the TV program brands (M Countdown, SBS Inkigayo, STUDIO
-- CHOOM) — a show is closer to a work than to the company behind it.
--
-- `list_assertions` learns the kind in the same change: these terms are
-- on Memories today as works, and a re-kind that silently removed them
-- would be a regression wearing a taxonomy fix. "A new kind is withheld
-- until somebody decides it belongs" — the owner just did.
--
-- Organizations take no bio frame: `bio_category` has no organization
-- branch, so they fall to `other`, which is silence — correct for a
-- company on a dating profile.
...
declare
  ...
  moved integer := 0;
  org_keys text[] := array[
    'work:big_hit_entertainment', 'work:hybe', 'work:hybe_labels',
    'work:sm_entertainment', 'work:yg_entertainment',
    'creator:jyp_entertainment',
    'work:kbs_world_tv', 'work:kbs_kpop', 'work:mbc', 'work:mnet',
    'work:disney', 'work:rolling_stone', 'work:rooster_teeth',
    'work:hsinchu_girls_high_school'];
begin
  select version, id into current_version, current_version_id
    from ontology.versions where status = 'published';
  if current_version is null then
    return;  -- nothing to re-kind; the replay is clean
  end if;
  if not exists (
    select 1 from ontology.concept_revisions cr
    join ontology.concepts c on c.id = cr.concept_id
   where cr.ontology_version_id = current_version_id and cr.status = 'active'
     and c.concept_key = any(org_keys)
     and cr.concept_kind <> 'organization') then
    return;  -- already moved; the replay is clean
  end if;
  ...
  values (gen_random_uuid(), next_version, current_version_id, 'draft',
          '0465: the labels and broadcasters re-kind to organization.')
  ...
  perform semantic_private.enqueue_recompute_on_analysis_change(
    'ontology ' || next_version || ': ' || moved
    || ' label(s) and broadcaster(s) re-kinded to organization');
end;
$$;
