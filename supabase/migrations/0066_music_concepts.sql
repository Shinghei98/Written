...
select gen_random_uuid(), '0.2.0', v.id, 'draft',
...
join ontology.versions old_v on old_v.id = r.ontology_version_id and old_v.version = '0.1.0'
cross join (select id from ontology.versions where version = '0.2.0') new_v
...
join ontology.versions old_v on old_v.id = l.ontology_version_id and old_v.version = '0.1.0'
cross join (select id from ontology.versions where version = '0.2.0') new_v
...
join ontology.versions old_v on old_v.id = e.ontology_version_id and old_v.version = '0.1.0'
cross join (select id from ontology.versions where version = '0.2.0') new_v
...
join ontology.versions old_v on old_v.id = m.ontology_version_id and old_v.version = '0.1.0'
cross join (select id from ontology.versions where version = '0.2.0') new_v
on conflict do nothing;
...
cross join (select id from ontology.versions where version = '0.2.0') v
on conflict (ontology_version_id, concept_id) do nothing;
...
select v.id, child.id, 'broader', parent.id, 1.0, 'curated',
       '{"source": "0066_music_concepts"}'::jsonb, 'active'
from seed_music s
...
update ontology.versions
   set status = 'retired'
 where version = '0.1.0' and status = 'published';

update ontology.versions
   set status = 'published', published_at = now()
 where version = '0.2.0' and status = 'draft';
