-- 0387 — labels get their shelves, albums follow their performers, and
--          the performer prefix reads the dictionary.
--
-- **Three owner directives, 2026-08-25:**
--
-- 1. **Two hardcoded card titles, by the owner's explicit word** —
--    `subject:music_labels` (hub:music) and
--    `subject:entertainment_labels` (hub:film_video, music labels
--    excluded). Organization-shaped concepts classify by their own
--    evidence: an org whose incoming promoted relations come from
--    music-blocked subjects is a music label; one fed by screen-blocked
--    subjects is an entertainment label; one with neither stays
--    unclassified and off the shelves ("others can be ditched").
-- 2. **An album or tour takes its performer's default genre** — the
--    performer's own specific heading, through the dictionary's
--    performed_by relation. An album is the performer's world; a tour
--    doubly so.
-- 3. **The performer prefix falls back to the dictionary's performed_by**
--    when the evidence payload carries no performer field (YouTube), so
--    "Next Level" reads "aespa - Next Level" like its Apple siblings.
...
begin
  ...
  values (extensions.gen_random_uuid(), next_version, old_version_id, 'draft',
          'Label shelves, album inheritance, performer-prefix fallback.');
  ...
  values (extensions.gen_random_uuid(), 'subject:music_labels'),
         (extensions.gen_random_uuid(), 'subject:entertainment_labels')
  ...
  values
    (new_version_id, music_id, 'Music Labels', 'topic',
     'Record labels and music agencies a person engages with.',
     'ordinary', 'review_required', 'active',
     '{"origin":"0387_label_shelves"}'::jsonb),
    (new_version_id, ent_id, 'Entertainment Labels', 'topic',
     'Broadcasters, studios and entertainment companies outside music.',
     'ordinary', 'review_required', 'active',
     '{"origin":"0387_label_shelves"}'::jsonb)
  ...
  values
    (new_version_id, music_id, 'Music Labels', 'music labels', 'en',
     'preferred', 'curated', 1.0, 'active', '{}'::jsonb),
    (new_version_id, ent_id, 'Entertainment Labels', 'entertainment labels',
     'en', 'preferred', 'curated', 1.0, 'active', '{}'::jsonb)
  ...
  select new_version_id, s.id, 'broader', h.id, 1.0, 'curated',
         '{"source":"0387_label_shelves"}'::jsonb, 'active'
  from (values ('subject:music_labels','hub:music'),
               ('subject:entertainment_labels','hub:film_video')) as m(sk, hk)
  join ontology.concepts s on s.concept_key = m.sk
  join ontology.concepts h on h.concept_key = m.hk
  on conflict do nothing;

  -- Classification by the org's own evidence.
  create temporary table _orgs on commit drop as
  select distinct c.id, c.concept_key,
         (select count(*) from ontology.concept_edges e
           join ontology.concepts s2 on s2.id = e.subject_concept_id
          where e.ontology_version_id = new_version_id and e.status = 'active'
            and e.object_concept_id = c.id and e.predicate_key <> 'broader'
            and semantic_private.concept_hub(s2.id, new_version_id) = 'hub:music')
           as music_feeders,
         (select count(*) from ontology.concept_edges e
           join ontology.concepts s2 on s2.id = e.subject_concept_id
          where e.ontology_version_id = new_version_id and e.status = 'active'
            and e.object_concept_id = c.id and e.predicate_key <> 'broader'
            and semantic_private.concept_hub(s2.id, new_version_id) = 'hub:film_video')
           as screen_feeders
  ...
  select new_version_id, o.id, 'broader',
         ...
         0.8, 'learned', '{"source":"0387_label_shelves"}'::jsonb, 'active'
    from _orgs o
   where o.music_feeders > 0 or o.screen_feeders > 0
  on conflict do nothing;
  get diagnostics n = row_count;
  raise notice '0387: % organization(s) shelved', n;

  -- 2. Albums and tours inherit the performer's heading.
  insert into ontology.concept_edges (
    ontology_version_id, subject_concept_id, predicate_key, object_concept_id,
    confidence, provenance_type, provenance, status)
  select distinct on (ac.id)
         new_version_id, ac.id, 'broader', h.id, 0.8, 'learned',
         '{"source":"0387_album_follows_performer"}'::jsonb, 'active'
    from semantic_private.presumed_terms t
    ...
    join semantic_private.presumed_terms pt on pt.id = rel.object_term_id
    join ontology.concepts pc on pc.id = pt.promoted_concept_id
    cross join lateral (
   ...
   order by ac.id, rel.observed_count desc nulls last
  on conflict do nothing;
  get diagnostics n = row_count;
  raise notice '0387: % album/tour(s) follow their performer', n;

  perform ontology.publish_version(new_version_id);
  perform semantic_private.enqueue_recompute_on_analysis_change(
    'ontology ' || next_version || ': label shelves and album inheritance');
  raise notice '0387: % published', next_version;
end;
$$;

-- 3. The prefix fallback (full function redefinition, two marked changes).
  ...
  left join lateral (
    select ot.canonical_label as artist
     ...
     where revision.concept_kind = 'work'
       ...
       and t.promoted_concept_id = assertion.concept_id
     order by rel.observed_count desc nulls last, ot.id
     limit 1
  ) as song2 on true
...
begin
  if position('order by coalesce(score.surfacing_score, 1.0) desc' in
    ...
    raise exception '0387: list_assertions lost its ordering';
  end if;
end;
$$;
