-- 0365 — the parenthesis is for a foreign original, not a short form.
--
-- 0364's first deployed page composed "Johann Sebastian Bach (Bach)",
-- "Chiquita (CHIQUITA)" and "i-dle (idle)" beside the intended
-- "Jay Chou (周杰倫)" — the dictionary's original_label sometimes carries
-- the surface as seen, which for a Western name is a short form of the
-- same name, not the entity's own-language name. The designated syntax is
-- for foreign originals, so the composition now requires a non-ASCII
-- original — which keeps every CJK name and diacritic-bearing title and
-- drops every same-script echo. One added condition, marked `-- 0365:`.
  ...
  left join lateral (
    select t.original_label
     ...
     where t.original_label is not null
       ...
       -- 0365: the parenthesis is for a *foreign* original. A pure-ASCII
       -- original_label is a short form or a case variant of the same name
       -- ("Bach", "idle", "CHIQUITA"), not another language's name for the
       -- entity — measured on the first deployed page.
       and t.original_label ~ '[^\x00-\x7F]'
...
begin
  if position('order by coalesce(score.surfacing_score, 1.0) desc' in
    ...
    raise exception '0365: list_assertions lost its ordering';
  end if;
end;
$$;
