-- 0375 — the bridge refuses hubs, the page drops the broad layers, and
--          the performer is found where extraction put the title.
--
-- **Three rule corrections from the owner's review (2026-08-25), no term
-- named:**
--
-- 1. **A bare hub is routing, not placement — and the bridge forgot.**
--    §2.15's rule, enforced in the assembler's offer() and violated by
--    0369: dictionary proposed-parents that were `hub:*` crossed as
--    edges (13 of them), which is how a song reads as "just music". The
--    edges reject here and the rule joins the bridge's contract.
-- 2. **place, travel and culture leave the Memories rows, every origin**
--    — too broad a layer; cultural preference is internal marking.
--    (Supersedes 0373's travel rows, same day, by the owner's word.)
-- 3. **The performer-prefix match honours extraction's own split.** The
--    artist is rightly split out of an album title, so exact
--    title-equality misses the rows whose performer is best known; the
--    match now accepts the album field, or containment with the
--    performer named in the remainder.
--
-- Recorded for the corpus cycle, not fixable at display: original_label
-- arriving as romaji (the grammar demands the entity's own script), the
-- five-family fracture of one referent (family reconciliation), and the
-- drama-heading hole (0346's kind-unguarded twin-merge folded the TV
-- genre onto subject:drama — the kind guard exists since 0356; the slice
-- re-fetch restores the heading).

begin;

-- 1. The 13 hub edges reject, at a new version, by rule.
do $$
declare
...
begin
  ...
  if not exists (
    ...
    where e.ontology_version_id = old_version_id and e.status = 'active'
      and e.provenance->>'source' = '0369_dictionary_bridge'
      and p.concept_key like 'hub:%') then
    raise notice '0375: no hub-parent bridge edges here; nothing to reject';
    return;
  end if;
  ...
  values (extensions.gen_random_uuid(), next_version, old_version_id, 'draft',
          'Bridge edges whose parent is a bare hub reject: routing is not placement.');
  ...
  update ontology.concept_edges e
     set status = 'rejected'
    from ontology.concepts p
   where p.id = e.object_concept_id
     and e.ontology_version_id = new_version_id and e.status = 'active'
     and e.provenance->>'source' = '0369_dictionary_bridge'
     and p.concept_key like 'hub:%';
  get diagnostics n = row_count;

  perform ontology.publish_version(new_version_id);
  perform semantic_private.enqueue_recompute_on_analysis_change(
    'ontology ' || next_version || ': ' || n || ' hub-parent bridge edge(s) rejected');
  raise notice '0375: % published — % bare-hub edge(s) rejected', next_version, n;
end;
$$;
...
begin
  if position('order by coalesce(score.surfacing_score, 1.0) desc' in
    ...
    raise exception '0375: list_assertions lost its ordering';
  end if;
end;
$$;
