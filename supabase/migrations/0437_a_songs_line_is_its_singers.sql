-- 0437 — a song's line on the page is its singer's.
--
-- **The owner's music ruling (2026-08-27): Memories shows persons
-- (singers, composers) and groups — not the albums and songs beneath
-- them — and striking the person strikes the works affiliated to
-- them.** Two rules, no lists:
--
--   * **The musical-work test is the artist resolution that already
--     exists** — the three laterals that compose "singer - song"
--     labels (accepted-mapping payload, the dictionary's performed_by,
--     the member-to-group resolution). A work that resolves an artist
--     is a musical work and leaves the page; Lucifer and Loki resolve
--     none and stay. One detector, already trusted for the labels,
--     now trusted for the filter — two copies of that decision would
--     drift.
--   * **The strike travels by derivation, not by cascade rows** —
--     scorer 0.23.0 expands the suppressed set with every work
--     reaching a suppressed creator via performed_by/composed_by
--     (concept edges and dictionary promoted links), so a new song by
--     a struck singer arrives already struck and restoring the person
--     restores the lot. The recompute is enqueued; the demotion sweep
--     retires the standing works of any already-struck creator.
...
begin
  ...
  if old_row.id is null then
    raise notice '0437: no active scorer stands; the model rows wait';
  else
    ...
    values (extensions.gen_random_uuid(), old_row.model_key,
            '0.23.0', 'scorer', 'active',
            coalesce(old_row.parameters, '{}'::jsonb)
              || jsonb_build_object('suppression_cascade',
                   'a suppressed creator derives suppression onto works '
                   || 'reaching them via performed_by/composed_by, through '
                   || 'concept edges and dictionary promoted links'));
  end if;
end;
$$;
...
begin
  if position('order by coalesce(score.surfacing_score, 1.0) desc' in
    ...
    raise exception '0437: list_assertions lost its ordering';
  end if;
  perform semantic_private.enqueue_recompute_on_analysis_change(
    '0437: music shows its people; strikes reach affiliated works');
end;
$$;
