-- 0403 — a deprecated revision answers nothing.
--
-- **Episode 25 stayed on the page after 0401 retired its revision**,
-- because `list_assertions` joins `concept_revisions` with no status
-- filter: a deprecated revision still supplied the `work` kind that
-- passes the allowlist and the label that draws the row. The whole
-- shadow-retirement mechanism rests on the reader honouring revision
-- status, and the reader never did — a check that can be skipped will be
-- skipped exactly when it is needed.
--
-- Two `status = 'active'` conditions close it: the concept's own
-- revision (no active revision -> no kind -> an inferred assertion is
-- withheld; a term the person typed still shows, per the standing rule)
-- and the block heading's revision. Everything else is 0391's text,
-- verbatim.
